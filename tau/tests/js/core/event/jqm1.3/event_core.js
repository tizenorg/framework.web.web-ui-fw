...
$().ready(function(){
	var libName = "jquery.mobile.events.js",
		components = [ "event/touch.js", "event/throttledresize.js", "event/orientationchange.js" ],
		absFn = Math.abs,
	...
	$.testHelper.excludeFileProtocol(function(){
		test( "new events defined on the jquery object", function(){
			$.each(events, function( i, name ) {
				delete $.fn[name];
				deepEqual($.fn[name], undefined, "new events defined on the jquery object");
			});
	...
	var nativeSupportTest = function(opts){
		...
		deepEqual($.event.special.orientationchange[opts.method](), opts.returnValue, 'nativeSupportTest');
	};
	...
	asyncTest( "throttledresize event prevents resize events from firing more frequently than one per 250ms", function(){
		...
		$.testHelper.sequence([
			...
			function(){ deepEqual( called, 1, 'called = 1' ); },
	...
	asyncTest( "mousedown mouseup and click events should add a which when its not defined", function() {
		var whichDefined = function( event ){
			deepEqual(event.which, 1, 'only one triggering');
		};
		...
		$( document ).bind( "vmouseup", function( event ){
			deepEqual(event.which, 1, 'only one triggering vmouseup');
			start();
		});
