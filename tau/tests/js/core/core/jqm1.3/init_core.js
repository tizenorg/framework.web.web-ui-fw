...
(function($){
	var loader, mobilePage = undefined,
			...
			extendFn = $.extend,
			setGradeA = function(value) { ej.support.gradeA = function(){ return value; }; },
	...
	$.testHelper.excludeFileProtocol(function(){
		$.testHelper.reloadLib = function () {ej.engine.run();};
		...
		test( "active page and start page should be set to the fist page in the selected set", function(){
			...
			deepEqual($.mobile.firstPage[0].id, firstPage[0].id, '');
			deepEqual($.mobile.activePage[0].id, firstPage[0].id, '');
		});
		...
		test( "mobile viewport class is defined on the first page's parent", function(){
			...
			ok(firstPage.parent().parent().hasClass("ui-mobile-viewport"), "first page has viewport");
		});
